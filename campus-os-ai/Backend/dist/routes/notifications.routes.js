...
notificationsRouter.get('/', asyncHandler(async (req, res) => {
    ...
    params.push(limit);
    const rows = await query(`SELECT id, type, title, message, is_read AS read, created_at AS "createdAt"
       FROM notifications
...
notificationsRouter.patch('/read-all', asyncHandler(async (req, res) => {
    const userId = req.user.id;
    const rows = await query('UPDATE notifications SET is_read = true WHERE user_id = $1 AND is_read = false RETURNING id', [userId]);
...
notificationsRouter.post('/', asyncHandler(async (req, res) => {
    ...
    const rows = await query(`INSERT INTO notifications (user_id, type, title, message)
       VALUES ($1, $2, $3, $4)
       RETURNING id, type, title, message, is_read AS read, created_at AS "createdAt"`, [userId, safeType, title.trim(), message?.trim() || null]);
...
notificationsRouter.patch('/:id/read', asyncHandler(async (req, res) => {
    ...
    const rows = await query(`UPDATE notifications SET is_read = true
       WHERE id = $1 AND user_id = $2
       RETURNING id, type, title, message, is_read AS read, created_at AS "createdAt"`, [id, userId]);
    if (!rows[0])
